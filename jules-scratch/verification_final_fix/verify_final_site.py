...
def run(playwright):
    ...
    try:
        # 1. Navigate to the home page using the correct port
        page.goto("http://localhost:34429", timeout=20000)
        ...
        page.screenshot(path="jules-scratch/verification_final_fix/01_homepage_final.png")
        ...
        expect(page).to_have_url("http://localhost:34429/kids-program", timeout=10000)
        expect(page.locator(".kids-hero-title")).to_be_visible()

        # Take screenshot of the Kids Program page
        page.screenshot(path="jules-scratch/verification_final_fix/02_kids_page_verified.png")

        # 4. Verify the color inversion on scroll
        page.evaluate("window.scrollBy(0, 500)")

        scrolled_app = page.locator(".App.scrolled")
        expect(scrolled_app).to_be_visible(timeout=5000)
        ...
        page.screenshot(path="jules-scratch/verification_final_fix/03_kids_program_scrolled.png")
